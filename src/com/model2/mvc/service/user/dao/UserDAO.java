	...
	public HashMap<String, Object> getUserList(SearchVO searchVO) throws Exception {
		...
		if (searchVO.getSearchCondition() != null) {
			if (searchVO.getSearchCondition().equals("0")) {
				sqlCondition += " WHERE USER_ID LIKE '%'||'" + searchVO.getSearchKeyword() + "'||'%' ";
			} else if (searchVO.getSearchCondition().equals("1")) {
				sqlCondition += " WHERE USER_NAME LIKE '%'||'" + searchVO.getSearchKeyword() + "'||'%' ";
			}
		}
		
		String sql = "select v2.*, v3.count from (select rownum rn, v1.* "
				+ " from (select * from users "+sqlCondition+" order by user_id)v1)v2,"
				+ " (select count(*) count from users"+sqlCondition+")v3 "
				+ "where v2.rn between ? and ? order by rn ";
